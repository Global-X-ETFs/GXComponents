  ...
  ({ className, border, ...props }, ref) => {
    return (
      <div ref={ref} className={cn(fundCardVariants({ border, className }))}>
        ...
        <div
          ref={ref}
          className="py-[12px] px-[15px] self-center overflow-hidden"
        >
          <h4
            ref={ref}
            className="text-base leading-6 text-gray-900 py-1 font-bold tracking-tight font-proxima"
          >
            {props.title}
          </h4>
          {/* sub title */}
          <h4
            ref={ref}
            className="text-4xl font-bold tracking-normal text-accent"
          >
            {props.subtitle}
          </h4>
        </div>
